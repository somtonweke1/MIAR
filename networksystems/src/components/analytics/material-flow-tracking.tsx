...
const MaterialFlowTracking: React.FC = () => {
  ...
  return (
    <div className="space-y-8">
              ...
              <div className="space-y-6">
                <div className="text-center">
                  ...
                  <div className="text-3xl font-light text-zinc-900 mb-1">
                    ${materials[selectedMaterial].currentPrice.toLocaleString()}
                  </div>
                  <div className={`text-sm flex items-center justify-center ${
                    getPriceChangeColor(materials[selectedMaterial].priceChange)
                  }`}>
                    {materials[selectedMaterial].priceChange >= 0 ? (
                      <TrendingUp className="h-4 w-4 mr-1" />
                    ) : (
                      <TrendingDown className="h-4 w-4 mr-1" />
                    )}
                    {Math.abs(materials[selectedMaterial].priceChange).toFixed(1)}% this month
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    <div>
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="font-medium">{(materials[selectedMaterial].supply.africa / 1000).toFixed(0)}K tonnes</span>
                      </div>
                    ...
                    <div>
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="font-medium">{(materials[selectedMaterial].demand.africa / 1000).toFixed(0)}K tonnes</span>
                      </div>
                      <div className="w-full bg-zinc-200 rounded-full h-2">
                ...
                <div className="bg-zinc-50 rounded-lg p-4">
                  ...
                  <div className="text-center">
                    <div className="text-2xl font-light text-zinc-900">
                      {materials[selectedMaterial].stockLevels.days_remaining}
                    </div>
                    <div className="text-sm text-zinc-600">days remaining</div>
                    <div className="text-xs text-zinc-500 mt-1">
                      {(materials[selectedMaterial].stockLevels.africa / 1000).toFixed(0)}K tonnes in stock
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-zinc-50 rounded-lg p-4">
                  ...
                  <div className="space-y-2">
                    {materials[selectedMaterial].supply.constraints.map((constraint, idx) => (
      ...
      {/* Forecast Chart */}
      {selectedMaterial && forecastData[selectedMaterial] && (
        <div className="bg-white/60 backdrop-blur-sm rounded-2xl border border-zinc-200/50 overflow-hidden shadow-xl shadow-zinc-200/20">
          <div className="border-b border-zinc-200/50 px-8 py-6">
            <h3 className="text-xl font-extralight text-zinc-900 tracking-tight">
              {materials[selectedMaterial]?.name} - 12-Month Supply Forecast
            </h3>
          </div>
          <div className="p-8">
            <div className="h-64 bg-gradient-to-br from-zinc-50 to-zinc-100 rounded-lg p-4">
              ...
              <div className="relative h-full">
                <svg className="w-full h-full">
                  {forecastData[selectedMaterial].map((point, idx) => {
                    const x = (idx / (forecastData[selectedMaterial].length - 1)) * 100;
                    const y = 100 - (point.value / Math.max(...forecastData[selectedMaterial].map(p => p.value))) * 80;
                    ...
                    return (
                      <g key={idx}>
                        ...
                        <rect
                          x={`${x - 0.5}%`}
                          y={`${100 - (point.confidence_interval.upper / Math.max(...forecastData[selectedMaterial].map(p => p.value))) * 80}%`}
                          width="1%"
                          height={`${((point.confidence_interval.upper - point.confidence_interval.lower) / Math.max(...forecastData[selectedMaterial].map(p => p.value))) * 80}%`}
                          fill="#3b82f6"
                          opacity="0.2"
                        />
                        {/* Data point */}
                  ...
                  {/* Trend line */}
                  <polyline
                    points={forecastData[selectedMaterial].map((point, idx) => {
                      const x = (idx / (forecastData[selectedMaterial].length - 1)) * 100;
                      const y = 100 - (point.value / Math.max(...forecastData[selectedMaterial].map(p => p.value))) * 80;
                      return `${x}%,${y}%`;
                    }).join(' ')}
